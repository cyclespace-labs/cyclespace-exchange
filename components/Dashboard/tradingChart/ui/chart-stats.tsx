...
export default function ChartStats({ tokenSymbol }: MarketStatsProps) {
  ...
  return (
    <div className=" w-full h-full items-center">
      <div className="px-6 w-full gap-2 h-full justify-between items-center flex flex-row">
...
function StatItem({ title, value, isCurrency }: { title: string; value: number | undefined; isCurrency: boolean }) {
  return (
    <div className="w-full h-15 p-2 rounded-2xl justify-between flex flex-col text-start items-center">
      <h1 className="text-[14px] text-start font-medium text-zinc-400">{title}</h1>
      <h2 className="text-[14px] text-start font-semibold">
        {value !== undefined
          ? isCurrency
